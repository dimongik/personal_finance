...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	табДок = Константы.ДанныеТабличногоДокумента.Получить().Получить();
	Если ТипЗнч(табДок) = Тип("ТабличныйДокумент") Тогда
		ТаблицаВспомогательная.Вывести(табДок);
	КонецЕсли;
...
&НаКлиенте
Процедура ТаблицаВспомогательнаяПриИзмененииСодержимогоОбласти(Элемент, Область, ДополнительныеПараметры)
	//Топорно, но пока так
	ТабДок = ТаблицаВспомогательная;
	СохранитьТабДокВБазу(ТабДок);
КонецПроцедуры
...
Процедура Подключаемый_ПриИзмененииРеквизита(Элемент)
	...
	ДанныеБюджета.СуммаПлан = СтрокаСтатьи.СуммаПлан;
	...
	СтрокаСтатьи.СуммаОстаток = СтрокаСтатьи.СуммаПлан - СтрокаСтатьи.СуммаФакт;
	ИтоговыеДанные = ИтоговыеДанныеЗаПериод(МесяцОтражения.ДатаНачала, МесяцОтражения.ДатаОкончания);
	ЗаполнитьЗначенияСвойств(Объект, ИтоговыеДанные);
...
Процедура ИнициализироватьСтатьЗатрат()
	...
	|	БюджетОбороты.ПланОборот КАК СуммаПлан,
	|	БюджетОбороты.ФактОборот КАК СуммаФакт,
	|	БюджетОбороты.ПланОборот - БюджетОбороты.ФактОборот КАК СуммаОстаток,
...
&НаСервереБезКонтекста
Процедура СохранитьТабДокВБазу(ТабДок)
	Хранилище = Новый ХранилищеЗначения(ТабДок);
	Константы.ДанныеТабличногоДокумента.Установить(Хранилище);
КонецПроцедуры
