...
&НаКлиенте
Функция ЗаполнитьДанныеПоСтатьеКлиент()
	
	ИмяСтатьи = ИмяСтатьяДляРазработчика(Объект.Затрата);
	СтрокаСтатьи = НайтиСтрокуСтатьиЗатратКлиент(ИмяСтатьи);
	Объект.ИтогоПоСтатье = 	СтрокаСтатьи.СуммаФакт;
...
Функция ИмяСтатьяДляРазработчика(Статья)
	
	Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Статья, "ИмяДляРазработчика");
